...
} from "/src/components/modals/Modal.jsx";
...
function GalleryModal({ displayingGallery, hideGallery }) {
  ...
  const tag = "gallery";
  const direction =
    aspectRatio === "16:9" && !isBreakpoint("xl") && !utils.isAndroid()
      ? "vertical"
      : "horizontal";
  ...
  return (
    <Modal id={`gallery-modal`} visible={Boolean(displayingGallery)}>
      {images && aspectRatio && (
        <ModalWindowTransparent transparent={true} onClose={_close}>
          <Swiper
            ...
            className={`gallery-swiper ${
              direction === "vertical" ? "gallery-swiper-no-bullets" : ""
            }`}
          >
