...
import ImageGallery from '/src/components/widgets/ImageGallery';
import MyCV from '/src/components/widgets/MyCV';
...
function Section({ section }) {
  ...
  const articles =
    section.content && section.content['articles'] ? section.content['articles'] : [];
          ...
          <div className={`lead-section-content`}>
            {settings['fullScreenButtonEnabled'] &&
              ...
              !utils.isSafari() && (
                ...
                >
                  <FullScreenToggleButton enabled={true} className={`fullscreen-toggle`} />
                </div>
              )}
          ...
          <div className={`article-wrapper ${mtClass}`} key={key}>
            ...
            {!Component && (
              <div className={`alert alert-danger text-3`}>
                Component <strong>{article.component}</strong> not found! Make sure the component
                exists and is listed on the <i>ARTICLES</i> dictionary on <b>Section.jsx</b>.
              </div>
            )}
          </div>
        );
